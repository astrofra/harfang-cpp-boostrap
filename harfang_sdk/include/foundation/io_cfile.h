...
/// C-File I/O
class CFile : public Driver {
public:
	CFile() {}
	CFile(const std::string &root_path, bool sandbox = false);

	void SetRootPath(const std::string &path, bool sandbox = false);
	
	std::string MapToAbsolute(std::string path) const override;
	std::string MapToRelative(std::string path) const override;
