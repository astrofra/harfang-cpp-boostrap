...
template <typename T> TypeId GetTypeId() { // coherent between DLLs, hash must not collide
...
template <typename T> void RegisterTypeReflection(TypeRegistry &registry) { __ASSERT__(false); }

//----
template <typename T> struct TSerializationStackEntry {
	const Type *type;
	T i;
...
template <typename T> struct TSerializationStack : std::vector<T> {
...
struct DeserializationStack : TSerializationStack<DeserializationStackEntry> {
	template <class C> C *FindInstance() const;
};

struct SerializationStack : TSerializationStack<SerializationStackEntry> {
	template <class C> const C *FindInstance() const;
};

template <class S> class TSerializationStackGuard {
	S *stack;
...
struct TypeSerializer {
	...
	virtual bool Deserialize(IDocumentReader &doc, void *o, const Type *type, DeserializationContext *ctx = nullptr, const char *name = nullptr) = 0;
	virtual bool Serialize(IDocumentWriter &doc, const void *o, const Type *type, SerializationContext *ctx = nullptr, const char *name = nullptr) = 0;
...
template <typename T> struct TypeFactory : ITypeFactory {
	explicit TypeFactory(const Type *t) : type(t) {}
...
public:
	...
	template <typename T> Type *HasType();
	template <typename T> Type *GetType();

	template <typename T> Type &CreateAbstractType();
	template <typename T> Type &CreateType();

	Type &CreateAlias(const char *alias, Type *);
...
public:
	...
	TypeValue(const Type *t, const void *o = nullptr);
	TypeValue(const char *type_name, const void *o = nullptr);
	template <typename T> TypeValue(const T &v);
	...
	//---------------------------------
	void Clear();
	bool Set(const Type *t, const void *in = nullptr);

	//---------------------------------
	template <typename T> bool Set(const T *);
	template <typename T> T &Cast(const Type *to) const;
	template <typename T> T &Cast() const;
	template <typename T> void Get(T &out) const { out = Cast<T>(); }

	//---------------------------------
	TypeValue &operator=(const TypeValue &b) {
		Set(b.GetType(), b.GetData());
		return *this;
	}
...
template <typename T> struct ConcreteTypeValueComparator : public TypeValueComparator {
...
public:
	...
	explicit TypeField(const char *hint_) : type(nullptr), hint(hint_) {}
	virtual ~TypeField() {}
	...
	template <typename T> bool GetValue(const void *i, T *out) const;
	template <typename T> bool SetValue(void *i, const T &v) const;

	template <typename T> void SetDefaultValue(const T &v);
...
template <typename Class, typename Type> struct ClassField : TypeField {
	ClassField(Type Class::*f);
	ClassField(Type Class::*f, const char *hint);
...
template <class Class, typename Type, class Accessor> struct ClassProperty : TypeField {
	ClassProperty();
	explicit ClassProperty(const char *hint);
...
struct TypeMethod {
	...
	virtual const Type *GetArgumentType(uint) const = 0;

	virtual bool Call(void *o, uint arg_count = 0, TypeValue **args = nullptr, TypeValue *r_value = nullptr) = 0;
};

template <typename ClassType> class ClassVoidMethodNoArg : public TypeMethod {
public:
	explicit ClassVoidMethodNoArg(void (ClassType::*m)()) : method(m) {}

	uint GetArgumentCount() const override { return 0; }
	const Type *GetReturnType() const override { return nullptr; }
	const Type *GetArgumentType(uint) const override { return nullptr; }

	bool Call(void *o, uint arg_count, TypeValue **args, TypeValue *r_value = nullptr) override {
		if (arg_count)
			return false;
...
template <typename ClassType, typename ReturnType> class ClassConstMethod : public TypeMethod {
public:
	...
	bool Call(void *o, uint arg_count, TypeValue **unused(args), TypeValue *r_value = nullptr) override {
		if (arg_count)
			return false;
...
//----
template <typename T> class EnumTypeSerializer : public TypeSerializer
{
public:
...
struct ContainerReadInterface {
	...
	virtual TypeValue Get(uint) const = 0;
	virtual const void *GetAddress(uint) const = 0;
...
struct ContainerWriteInterface {
	...
	virtual bool Reserve(uint) = 0;
	virtual void Set(uint, const TypeValue &) = 0;
	virtual void *GetAddress(uint) const = 0;
...
struct DictionaryReadInterface {
	...
	virtual uint GetSize() const = 0; // number of keys in the dictionary
	virtual std::string GetKey(uint) const = 0;
...
struct EnumCast {
	...
	virtual TypeValue ToEnum(uint) = 0;
	virtual uint FromEnum(const TypeValue &) = 0;
};

template <typename T> struct ConcreteEnumCast : public EnumCast {
	explicit ConcreteEnumCast(TypeRegistry &r) : registry(r) {}

	TypeValue ToEnum(uint v) override {
		T e = static_cast<T>(v);
		return TypeValue(registry.GetType<T>(), &e);
	}

	uint FromEnum(const TypeValue &v) override { return uint(v.Cast<T>()); }
...
template <typename T> class ConcreteSharedPtrAccessor : public SharedPtrAccessor {
public:
...
public:
	...
	typedef std::map<std::string, uint> EnumMap;

private:
	TypeRegistry *registry;
	std::unique_ptr<ITypeFactory> factory;
	...
	template <typename T, typename B> struct ConcreteTypeCaster : public TypeCaster {
...
public:
	...
	inline const ITypeFactory *GetFactory() const { return factory.get(); }
	...
	//---------------------------------
	template <typename T> Type &SetSharedPtr() {
	...
	//---------------------------------
	template <typename T, typename B> Type &SetBase() {
		base_types.emplace_back(registry->GetType<B>(), new ConcreteTypeCaster<T, B>);
		return *this;
	}

	template <typename T, typename B0, typename B1> Type &SetBase() {
	...
	template <typename T, typename B0, typename B1, typename B2> Type &SetBase() {
	...
	inline uint GetBaseTypeCount() const { return base_types.size(); }
	inline Type *GetBaseType(uint index) const { return base_types[index].type; }

	bool Inherits(const Type *t) const {
		if (CompareType(this, t))
			return true;
		for (uint n = 0; n < base_types.size(); ++n)
			if (base_types[n].type->Inherits(t))
				return true;
		return false;
	}

	template <typename T> bool Inherits() const { return Inherits(registry->GetType<T>()); }
	...
	template <typename T> void *Downcast(T *p) const { return Downcast(p, registry->GetType<T>()); }
	template <typename T> const void *Downcast(const T *p) const { return Downcast(p, registry->GetType<T>()); }
	...
	template <typename T> void *Upcast(T *p) const { return Upcast(p, registry->GetType<T>()); }
	template <typename T> const void *Upcast(const T *p) const { return Upcast(p, registry->GetType<T>()); }

	void *Cast(void *p, const Type *to) const;
	const void *Cast(const void *p, const Type *to) const { return Cast(const_cast<void *>(p), to); }

	template <typename T> T *Cast(void *p) const {
		auto to = registry->GetType<T>();
		__ASSERT__(to != nullptr);
		return reinterpret_cast<T *>(Cast(p, to));
	}

	template <typename T> const T *Cast(const void *p) const { return Cast<T>(const_cast<void *>(p)); }

	//---------------------------------
	inline const std::string &GetRootScopeName() const { return root_scope; }

	Type &SetRootScopeName(const char *n) {
		root_scope = n;
		return *this;
	}

	//---------------------------------
	Type &AddField(const char *field_name, TypeField *field, bool serialized = true, bool muted = false) {
	...
	Type &AddFieldSetSerializationModifier(const char *field_name, TypeField *field, TypeFieldSerializationModifier *modifier, bool serialized = true, bool muted = false) {
		AddField(field_name, field, serialized, muted);
		field->SetSerializationModifier(modifier);
		return *this;
	}

	template <typename T> Type &AddFieldDefault(const char *field_name, TypeField *field, const T &default_value, bool serialized = true, bool muted = false) {
		AddField(field_name, field, serialized, muted);
		field->SetDefaultValue(default_value);
		return *this;
	}

	template <typename T> Type &AddFieldDefaultSetSerializationModifier(const char *field_name, TypeField *field, const T &default_value, TypeFieldSerializationModifier *modifier, bool serialized = true, bool muted = false) {
		AddField(field_name, field, serialized, muted);
		field->SetDefaultValue(default_value);
		field->SetSerializationModifier(modifier);
		return *this;
	...
	TypeField *GetField(const char *field_name) const {
		auto i = fields.find(field_name);
		return i != fields.end() ? (*i).second.get() : nullptr;
	}

	FieldMap::const_iterator GetField(uint idx) const {
		auto i = fields.begin();
		std::advance(i, idx);
		return i;
	}
	...
	//---------------------------------
	Type &AddMethod(const char *method_name, TypeMethod *method) {
		methods[method_name].reset(method);
		return *this;
	}

	TypeMethod *GetMethod(const char *method_name) const {
	...
	//---------------------------------
	template <typename T> Type &SetEnum() {
	...
	template <typename T> Type &AddValue(const char *enum_value_name, T enum_value) {
		values[enum_value_name] = uint(enum_value);
		return *this;
	}

	template <typename T> Type &AddValue(const std::string &enum_value_name, T enum_value) { return AddValue<T>(enum_value_name.c_str(), enum_value); }
	...
	//---------------------------------
	Type(TypeRegistry *registry_, const char *name_) : registry(registry_), factory(nullptr), name(name_), size(0) {}
};
...
//----
template <typename T> Type *TypeRegistry::HasType() { return HasType(GetTypeName<T>()); }
template <typename T> Type *TypeRegistry::GetType() { return GetType(GetTypeName<T>()); }
template <typename T> Type &TypeRegistry::CreateAbstractType() { return *GetType(GetTypeName<T>()); }

template <typename T> Type &TypeRegistry::CreateType() {
	Type &type = CreateAbstractType<T>();
	type.factory.reset(new TypeFactory<T>(&type));
	type.size = sizeof(T);
	return type;
}

//----
template <typename T> bool TypeField::GetValue(const void *i, T *v) const {
...
template <typename T> bool TypeField::SetValue(void *i, const T &v) const {
	if (type != type->GetRegistry()->GetType<T>())
		return false;
	Set(i, &v);
	return true;
...
template <typename T> void TypeField::SetDefaultValue(const T &v) {
...
//----
template <typename Class, typename Type> void ClassField<Class, Type>::Get(const void *i, void *v) const {
	*reinterpret_cast<Type *>(v) = reinterpret_cast<const Class *>(i)->*p;
}

template <typename Class, typename Type> void ClassField<Class, Type>::Set(void *i, const void *v) const {
	reinterpret_cast<Class *>(i)->*p = *reinterpret_cast<const Type *>(v);
}

template <typename Class, typename Type> void *ClassField<Class, Type>::GetAddress(void *o) const { return static_cast<void *>(&(reinterpret_cast<Class *>(o)->*p)); }
template <typename Class, typename Type> const void *ClassField<Class, Type>::GetAddress(const void *o) const { return static_cast<const void *>(&(reinterpret_cast<const Class *>(o)->*p)); }

template <typename Class, typename Type> ClassField<Class, Type>::ClassField(Type Class::*f) : p(f), size(sizeof(Type)) { type_name = gs::GetTypeName<Type>(); }
template <typename Class, typename Type> ClassField<Class, Type>::ClassField(Type Class::*f, const char *h) : TypeField(h), p(f), size(sizeof(Type)) { type_name = gs::GetTypeName<Type>(); }

//----
template <class Class, typename Type, class Accessor> void ClassProperty<Class, Type, Accessor>::Get(const void *i, void *v) const {
	*reinterpret_cast<Type *>(v) = Accessor::Get(*reinterpret_cast<const Class *>(i));
}

template <typename Class, typename Type, class Accessor> void ClassProperty<Class, Type, Accessor>::Set(void *i, const void *v) const {
	Accessor::Set(*reinterpret_cast<Class *>(i), *reinterpret_cast<const Type *>(v));
}

template <class Class, typename Type, class Accessor> ClassProperty<Class, Type, Accessor>::ClassProperty() { type_name = gs::GetTypeName<Type>(); }
template <class Class, typename Type, class Accessor> ClassProperty<Class, Type, Accessor>::ClassProperty(const char *h) : TypeField(h) {
	type_name = gs::GetTypeName<Type>();
}

//----
template <typename T> bool TypeValue::Set(const T *in) { return Set(g_type_registry.get().GetType<T>(), in); }

template <typename T> T &TypeValue::Cast(const Type *to) const {
	__ASSERT__(type != nullptr);
	__ASSERT_MSG__(type == to, "invalid TypeValue cast");
	return *reinterpret_cast<T *>(data);
}

template <> int &TypeValue::Cast<int>(const Type *to) const;

template <typename T> T &TypeValue::Cast() const { return Cast<T>(type->GetRegistry()->GetType<T>()); }

template <typename T> TypeValue::TypeValue(const T &v) : type(nullptr), data(nullptr) { Set(g_type_registry.get().GetType<T>(), &v); }

template <typename T> TypeValue MakeTypeValue(TypeRegistry &r, const T &v) { return TypeValue(g_type_registry.get().GetType<T>(), &v); }
template <typename T> TypeValue MakeTypeValue(const T &v) { return TypeValue(g_type_registry.get().GetType<T>(), &v); }

//----
template <typename T> bool ConcreteTypeValueComparator<T>::Compare(const TypeValue &a, const TypeValue &b) const { return a.GetType() == b.GetType() && a.GetType() == a.GetType()->GetRegistry()->GetType<T>() && *reinterpret_cast<const T *>(a.GetData()) == *reinterpret_cast<const T *>(b.GetData()); }

//----
template <typename T> bool EnumTypeSerializer<T>::Deserialize(IDocumentReader &doc, void *o, const Type *type, DeserializationContext *unused(ctx), const char *name) {
	__ASSERT__(type->IsEnum());
	std::string v;
	if (!doc.Read(name, v))
		return false;

	auto p = type->GetValues().find(v.c_str()); // FIXME v should be a std::string
...
template <typename T> bool EnumTypeSerializer<T>::Serialize(IDocumentWriter &doc, const void *o, const Type *type, SerializationContext *unused(ctx), const char *name) {
	__ASSERT__(type->IsEnum());
	auto &values = type->GetValues();
	uint v = static_cast<uint>(*reinterpret_cast<const T *>(o));
	auto i = std::find_if(values.begin(), values.end(), [v](const std::pair<std::string, uint> &pair) { return pair.second == v; });
	return i != values.end() && doc.Write(name, i->first.c_str());
}

//----
template <class C> C *DeserializationStack::FindInstance() const {
...
template <class C> const C *SerializationStack::FindInstance() const {
	static const auto type = g_type_registry.get().GetType<C>();
	auto entry = this->FindType(type);
