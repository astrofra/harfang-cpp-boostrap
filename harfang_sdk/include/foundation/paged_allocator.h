...
template <size_t BlockSize, uint16_t PageSize> struct AllocatorPage {
	AllocatorPage() : next(0), usage(0) {
	...
	uint16_t next; // next block index
	uint16_t usage;
};

template <size_t BlockSize, uint16_t PageSize> class PagedAllocator {
	typedef AllocatorPage<BlockSize, PageSize> Page;

public:
	PagedAllocator() : pages(8) { NewPage(); }
