...
enum ComponentState : uint8_t { NotReady, Ready, Failed };
...
public:
	...
	virtual std::shared_ptr<render::IRenderable> GetIRenderable() const { return nullptr; }
	/// Return the frame renderer interface if the component implements it.
	virtual std::shared_ptr<IFrameRenderer> GetIFrameRenderer() const { return nullptr; }
...
template <typename C, typename T> T DefaultValidator(C *, const T &v) { return v; }

template <typename C, typename T> T ClampToZero(C *, const T &v) { return v > 0 ? v : 0; }
template <typename C, typename T> T ClampToZeroOne(C *, const T &v) { return types::Clamp<T>(v, 0, 1); }

template <typename C> Vector4 ClampVector4ToZeroOne(C *, const Vector4 &v) {
	return Vector4(types::Clamp(v.x, 0.f, 1.f), types::Clamp(v.y, 0.f, 1.f), types::Clamp(v.z, 0.f, 1.f), types::Clamp(v.w, 0.f, 1.f));
}
...
//------------------------------------------------------------------------------
#define decl_component_base_defaults(ASPECT)        \
public:                                             \
...
#define decl_component_defaults(CLASS, ASPECT) \
public:                                        \
	decl_component_base_defaults(ASPECT)       \
                                               \
		static sComponent                      \
		Make() {                               \
