...
template <class T> void DrawRectPackerSplit(typename RectPacker<T>::split_index idx, Rect<T> cell_rect, const std::vector<typename RectPacker<T>::Split> &splits, Picture &pic)
{
	if (splits[idx].leaf)
		return;
...
template <class T> void DrawRectPackerSplits(Rect<T> root_rect, const std::vector<typename RectPacker<T>::Split> &splits, Picture &pic)
{
